...
void drawShape(enum Shape shape, int x_pos, int y_pos, int x_size, int y_size,
	...
	if (shape == Triangle) {
		...
		/*if (shape == Circle) {
			int radius = x_size;
			int xmiddle = x_pos + radius;
			int ymiddle = y_pos + radius;
			for (int y = y_pos; y < y_pos + y_size; y++) {
				for (int x = x_pos; x < x_pos + x_size; x++) {
					// Berechnen des Abstands vom Mittelpunkt
					float distance = sqrt(
							(x - radius) * (x - radius)
									+ (y - radius) * (y - radius));
					if (distance <= radius) {
	...
	int main(int argc, char *argv[]) {
		...
		if (fbp == (unsigned char*) -1) {
			...
			memset(fbp, 0x7f, screensize);
			for (int i = 0; i < 3; i++) {
				drawShape(Rectangle, 50 + 50 * i, 50 + 50 * i, 70, 30, 255, 0,
						255, 255);
			}
			for (int i = 0; i < 3; i++) {
				drawShape(Triangle, 300 + 50 * i, 50 + 50 * i, 50, 50, 0, 136,
						0, 255);
			}
			drawShape(Circle, 500, 100, 50, 50, 255, 255, 255, 255);
			sleep(5);
		}
