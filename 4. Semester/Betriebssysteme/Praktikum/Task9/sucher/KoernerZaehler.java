...
class KoernerZaehler extends SucherImpl implements Callable<Integer> {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < ANZAHL_SUCHER; i++) {
			int[] startPunkt = { i, 0 };  // Start jeweils am Anfang der Zeile
			Callable<Integer> sucher = new KoernerZaehler(territorium, startPunkt, Richtung.RECHTS);
			Future<Integer> future = pool.submit(sucher);
	...
	private void bearbeiteKachel() {
		...
		int[] pos = gibPosition();
		int koerner = gibTerritorium().gibKoerner(pos);
		gesamtZahlKoerner += koerner;
