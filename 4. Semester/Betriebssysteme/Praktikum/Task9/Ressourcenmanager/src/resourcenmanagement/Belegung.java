...
// Import entfernt, da eigene Beleger-Klasse im gleichen Package existiert
		...
		if (this.dieBeleger.isEmpty()) {
			...
			for (Beleger einBeleger : this.dieBeleger) {
				...
				if (einBeleger.gibRestforderung() <= zahlFreierBetriebsmittel) {
					...
					Set<Beleger> dieNeueBelegung = new HashSet<>(this.dieBeleger);
					// den Beleger, der fertig ist, aus dieNeueBelegung entfernen
					dieNeueBelegung.remove(einBeleger);
					...
					this.kindbelegung = new Belegung(dieNeueBelegung);
