...
hfu::City::City(const City& other)
    : City(other.position, other.pois.get(), other.number_of_poi) {}


std::unique_ptr<std::string[]> hfu::City::initializepois(const std::string *pois, const int length) {
    ...
    std::unique_ptr<std::string[]> poi(new std::string[length]);
...
void hfu::City::resize_array(int new_size) {
    ...
    std::unique_ptr<std::string[]> new_string(new std::string[new_size]);

    for (int i = 0; i < this->number_of_poi; i++) {
        new_string[i] = this->pois[i];
    }

    this->pois = std::move(new_string);
...
hfu::City& hfu::City::operator=(const hfu::City& other) {
    if (this == &other) {
        return *this;
    }

    this->position = other.position;
    this->number_of_poi = other.number_of_poi;
    this->length = other.length;
    this->pois = initializepois(other.pois.get(), other.number_of_poi);
